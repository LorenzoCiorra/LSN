...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        for (int j = 0; j < T; j++) {
            ...
            double norm = a*a + b*b;
            if(norm <= 1){
                x2 = x1+ l* a/sqrt(norm); //x coordinate of the end of the needle; we don't need its y. a/sqrt(norm)=cos(theta)
                // now that we have the needle on our table, we should check if it touches, or not, one of the two vertical planes
                if ( x2 >= d || x2 <= 0 ) {
                    n_hit++;
                }
            } else{ //the point (a, b) is not in the circle, we reject the throw and we try it again
                j--;
            }
        }
