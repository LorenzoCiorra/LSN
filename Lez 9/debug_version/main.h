...
int nr_cities=34;
int nr_genes=34;
int nr_chromosomes=500; //500 e 2000
double crossover_prob = 0.7, mutation_prob = 0.05, selection_prob = 0.1;
...
class genetic_functions { //la creo su un cromosoma e ci posso applicare le mutazioni. Per il crossover gli do' come argomento il secondo cromosoma
	...
	void shift_of_n (double probability) {
		if(m_rnd.Rannyu() < probability) {
			...
			//il +1 serve per non muovere mai il primo gene (prova_shift su replit)
		}
	}
	...
	void m_permutation (double probability) { //permuto a blocchi, praticamente e' uno swap tra blocchi con una lunghezza diversa da 1 (i.e. il singolo elemento)
		if(m_rnd.Rannyu() < probability) {
			...
			rotate (m_chromosome.begin()+block_start, m_chromosome.begin()+n+block_start, m_chromosome.begin()+block_finish); //funziona! (prova/m_permutation) METTERE +1 AL BLOCK_FINISH SE DA ERRORE
...
/*          cout << "goodpart1st" << endl;
            for(auto a=0; a<goodpart_1st.size();a++ ) {
                cout << goodpart_1st[a] << " ";
            }
            cout << endl;
*/
