...
int main (int argc, char *argv[]){
	...
	for (int i =0; i<N; i++) { //cycle on the blocks
		...
		// saving on an external file result of the inetgral and its error sampling a uniform distribution 
        uniform_results << sum_unif/(i+1) << " " << error(sum_unif/(i+1),error_unif/(i+1),i) <<endl;
		// saving on an external file result of the inetgral and its error sampling a non-uniform distribution 
