...
Random rnd;
std::string borders, rank_string, chromosome_number_string; //borders=opened vuol dire che voglio migrazione dei migliori cromosomi tra i continenti
double x, y, theta;
int nr_cities=50;
int nr_genes=50;
int nr_chromosomes=2000; //2000
...
unsigned int nr_max_generations = 300, migration_generation= 50; //ogni 50 generazioni abbiamo una migrazione
int generation_index = 0, migration_index=0;
double average_distance=0.;
ofstream out_cartesian_path, out_distance, out_average_distance;

ifstream american_capitals, list_of_capitals;
vector<unsigned int> ambassador_sender(nr_genes), ambassador_addressee(nr_genes);
//unsigned int* ambassador_sender = new unsigned int [nr_genes];
//unsigned int* ambassador_addressee = new unsigned int [nr_genes];
...
class fitness { //prende un vettore di citta' cosi' da calcolare per un cromosoma, che passo come argomento della funzione path, la sua funzione fitness
	private:
	vector <city> m_city;
	...
	public:
  	...
  	double path_fitness(vector <unsigned int> chromosome) {
	    ...
	    //percio' il cromosoma con distanza tot piu' piccola e' quello con migliore fitness
	}
};
...
class genetic_functions { //la creo su un cromosoma e ci posso applicare le mutazioni. Per il crossover gli do' come argomento il secondo cromosoma
	...
	void shift_of_n (double probability) {
		if(m_rnd.Rannyu() < probability) {
			...
			//il +1 serve per non muovere mai il primo gene (funziona! prova/shift_n)
		}
	}
	...
	void m_permutation (double probability) { //permuto a blocchi, praticamente e' uno swap tra blocchi con una lunghezza diversa da 1 (i.e. il singolo elemento)
		if(m_rnd.Rannyu() < probability) {
			...
			rotate (m_chromosome.begin()+block_start, m_chromosome.begin()+n+block_start, m_chromosome.begin()+block_finish); //(funziona! prova/m_permutation)
	...
	void inversion (double probability) { //inverto i geni di un blocco di lunghezza estratta
		...
		unsigned int block_finish = (unsigned int) m_rnd.Rannyu(block_start +1, m_chromosome.size());
		reverse (m_chromosome.begin()+block_start, m_chromosome.begin()+block_finish);
	}


	vector <vector<unsigned int>> crossover(vector <unsigned int> chromosome_2) { //la prob la metto nel main.cpp
		...
		for(auto i =0; i<m_chromosome.size(); i++) { //questo for deve andare su tutto il vettore iniziale nel caso in cui lo stesso numero si trovi nelle parti tagliate di entrambi i vettori
			for(auto j =0; j<m_chromosome.size()-path_cut; j++) {
